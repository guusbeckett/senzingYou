...
public class Camera
{
	...
	public Camera()
	{
		...
		try
		{
			...
			boolean hasAltView = depthGenerator
					.isCapabilitySupported("AlternativeViewPoint");
			if (hasAltView)
			{
				AlternativeViewpointCapability altViewCap = depthGenerator
						.getAlternativeViewpointCapability();
			...
			userGenerator.getNewUserEvent().addObserver(
					new IObserver<UserEventArgs>()
					{
								...
								UserEventArgs arg1)
						{
							try
							{
								if (!skeletonCapability
										.needPoseForCalibration())
								{
									skeletonCapability
											.requestSkeletonCalibration(
													arg1.getId(), true);
								}
							...
							getUsers().add(
									new User(arg1.getId(), userGenerator));
						}
					});

			// Check if the user re-entered
			userGenerator.getUserExitEvent().addObserver(
					new IObserver<UserEventArgs>()
					{
						@Override
						public void update(IObservable<UserEventArgs> arg0,
			...
			handsGenerator.getHandCreateEvent().addObserver(
					new IObserver<ActiveHandEventArgs>()
					{
						@Override
						public void update(
								IObservable<ActiveHandEventArgs> arg0,
			...
			// Updating the hand
			handsGenerator.getHandUpdateEvent().addObserver(
					new IObserver<ActiveHandEventArgs>()
					{
						@Override
						public void update(
			...
			handsGenerator.getHandDestroyEvent().addObserver(
					new IObserver<InactiveHandEventArgs>()
					{
						@Override
						public void update(
								IObservable<InactiveHandEventArgs> arg0,
								InactiveHandEventArgs arg1)
						{
							if (!getUsers().isEmpty())
							{
								try
								{
									handsGenerator.StartTracking(getUsers()
											.get(0).getRightHandWorld());
								} catch (StatusException e)
								{
					...
					new IObserver<CalibrationProgressEventArgs>()
					{
								...
								CalibrationProgressEventArgs arg1)
						{
							try
							{
								...
								{
									skeletonCapability.startTracking(arg1
											.getUser());
									handsGenerator.StartTracking(skeletonCapability
	...
	private BufferedImage getImageRGB()
	{
		...
		int g = 0;
		int b = 0;
		ByteBuffer rgbBuffer = imageGenerator.getMetaData().getData()
				.createByteBuffer();
		...
		image.setRGB(0, 0, VIEW_WIDTH, VIEW_HEIGHT, imageRGBArray, 0,
				VIEW_WIDTH);
		return image;
	}
	...
	private void getMeasurements(User user)
	{
		try
		{
			...
			double leftHandElbow = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_HAND).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_ELBOW).getPosition());
			double rightHandElbow = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_HAND).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_ELBOW).getPosition());
			double leftElbowShoulder = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_ELBOW).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_SHOULDER).getPosition());
			double rightElbowShoulder = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_ELBOW).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_SHOULDER).getPosition());
			double leftFootKnee = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_FOOT).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_KNEE).getPosition());
			double rightFootKnee = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_FOOT).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_KNEE).getPosition());
			double leftKneeHip = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_KNEE).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_HIP).getPosition());
			double rightKneeHip = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_KNEE).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_HIP).getPosition());
			double shoulder = distance(
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_SHOULDER).getPosition(),
					skeletonCapability.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_SHOULDER).getPosition());

			Recognition recog = new Recognition(leftHandElbow, rightHandElbow,
					leftElbowShoulder, rightElbowShoulder, leftFootKnee,
					rightFootKnee, leftKneeHip, rightKneeHip, shoulder,
					"Scanner");

			// System.out.println(recog);

			ArrayList<Recognition> recogs = new ArrayList<Recognition>();
			recogs.add(new Recognition(387.156991895671, 362.86219911934603,
					289.9022489645288, 247.87892129539745, 400.89900908176656,
					400.89901113437736, 416.93502701957476, 416.93500264395664,
					219.95290085537002, "Guus"));
			recogs.add(new Recognition(345.11650094536634, 359.1939648296733,
					239.83571686168503, 217.9514548108559, 338.74434258901204,
					338.74439159563866, 352.29415490163325, 352.2941458232829,
					277.5311020488579, "JW"));
			recogs.add(new Recognition(307.2743321928763, 273.26605097189434,
					234.52670942610283, 244.2972030081929, 354.84515516740237,
					354.8450460962978, 369.03894354930435, 369.03895263433446,
					396.54826718060207, "Sven"));
			recogs.add(new Recognition(342.32574430381396, 320.5748602741389,
					239.16373011038402, 230.1869826676575, 339.31738759009187,
					339.3173290627665, 352.89005446221404, 352.8900614152893,
					332.22212317512304, "Johan"));
	...
	private void setUserSkeleton(User user)
	{
		try
		{
			...
			user.setHead(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(), SkeletonJoint.HEAD)
					.getPosition()));
			user.setNeck(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(), SkeletonJoint.NECK)
					.getPosition()));

			user.setLeftShoulder(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_SHOULDER).getPosition()));
			user.setRightShoulder(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_SHOULDER).getPosition()));

			user.setTorso(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(), SkeletonJoint.TORSO)
					.getPosition()));

			user.setLeftElbow(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_ELBOW).getPosition()));
			user.setRightElbow(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_ELBOW).getPosition()));

			user.setLeftHand(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_HAND).getPosition()));
			user.setRightHand(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_HAND).getPosition()));

			user.setLeftHip(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_HIP).getPosition()));
			user.setRightHip(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_HIP).getPosition()));

			user.setLeftKnee(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_KNEE).getPosition()));
			user.setRightKnee(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_KNEE).getPosition()));

			user.setLeftFoot(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.LEFT_FOOT).getPosition()));
			user.setRightFoot(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(),
							SkeletonJoint.RIGHT_FOOT).getPosition()));

			user.setMidpoint(convertPosition(skeletonCapability
					.getSkeletonJointPosition(user.getId(), SkeletonJoint.TORSO)
					.getPosition()));
			user.setRightHandWorld(skeletonCapability.getSkeletonJointPosition(
					user.getId(), SkeletonJoint.RIGHT_HAND).getPosition());
